...
public class ApplicationConfig {
	...
	@Bean
	public SecurityFilterChain filterChain(HttpSecurity http, RoleBasedLoginSuccessHandler successHandler,
...
//	        )
				...
				.authorizeHttpRequests(auth -> auth
						...
						.permitAll().requestMatchers("/api/feedback/submit", "/api/subscribe").permitAll()
						.requestMatchers("/api/feedback/all").hasRole("ADMIN").requestMatchers("/admin/**")
						.hasRole("ADMIN").requestMatchers("/dashboard", "/dashboard/**", "/quiz/**", "/quizzes")
				...
				.formLogin(
						login -> login
								.loginPage("/auth/login").loginProcessingUrl("/auth/login").successHandler(
										successHandler)
								.failureHandler(failureHandler).permitAll())
				.oauth2Login(oauth2 -> oauth2.loginPage("/auth/login").userInfoEndpoint(userInfo -> userInfo

						.oidcUserService(oauth2UserService)).successHandler(successHandler)
